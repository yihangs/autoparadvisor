...
from bo.localbo_utils import from_unit_cube, latin_hypercube, to_unit_cube
from bo.localbo_utils import onehot2ordinal
    ...
    *Note*: in general, you should have the first d_cat dimensions as cat_dims, and the rest as the continuous dims.
    ...
    def __init__(
        ...
        if(self.int_constrained_dims is not None):
            self.integer_dims = np.where(np.in1d(self.cont_dims, self.int_constrained_dims))[0]
            self.pure_cont_dims = np.setdiff1d(np.arange(len(self.cont_dims)),self.integer_dims)
        else:
            self.integer_dims = None
            self.pure_cont_dims = np.arange(len(self.cont_dims))

    def _adjust_length(self, fX_next):
        print("Start to adjust TR length...")
        print(self.failcount,self.failtol)
    ...
    def _create_and_select_candidates(self, X, fX, length, n_training_steps, hypers, return_acq=False):
        ...
        #integer_dims = np.where(np.in1d(self.cont_dims, self.int_constrained_dims))[0]
        #pure_cont_dims = np.setdiff1d(np.arange(len(self.cont_dims)),integer_dims)
        if(self.integer_dims is not None):
            integer_range = np.round(from_unit_cube(np.vstack([lb[self.integer_dims],ub[self.integer_dims]]),self.lb[self.integer_dims],self.ub[self.integer_dims]))
        ...
        #def thompson_with_integer(n_cand=500):
        ...
        def _ei(X, augmented=True):
            ...
            #with torch.no_grad(), gpytorch.settings.max_cholesky_size(self.max_cholesky_size), gpytorch.settings.fast_pred_var():
