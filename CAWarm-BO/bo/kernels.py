...
def wrap(x1, x2, integer_dims):
    """The wrapping transformation for integer dimensions according to Garrido-Merchán and Hernández-Lobato (2020)."""
    if integer_dims is not None:
        x1[:,integer_dims] = torch.round(x1[:,integer_dims])
        x2[:,integer_dims] = torch.round(x2[:,integer_dims])
    return x1, x2


def wrap_integer(x1,x2,integer_dims,cont_lb,cont_ub):
    if integer_dims is not None:
        x1[:,integer_dims] = torch.round(from_unit_cube(x1[:,integer_dims],cont_lb[integer_dims],cont_ub[integer_dims]))
        x1[:,integer_dims] = to_unit_cube(x1[:,integer_dims],cont_lb[integer_dims],cont_ub[integer_dims])
        x2[:,integer_dims] = torch.round(from_unit_cube(x2[:,integer_dims],cont_lb[integer_dims],cont_ub[integer_dims]))
        x2[:,integer_dims] = to_unit_cube(x2[:,integer_dims],cont_lb[integer_dims],cont_ub[integer_dims])
    return x1,x2
...
class WrappedMatern(MaternKernel):
    ...
    def __init__(self, integer_dims=None, cont_lb=None, cont_ub=None, **kwargs):
    ...
    def forward(self, x1, x2, diag=False, **params):
        #pdb.set_trace()
        x1, x2 = wrap_integer(x1, x2, self.integer_dims,torch.tensor(self.cont_lb).to(x1),torch.tensor(self.cont_ub).to(x1))
