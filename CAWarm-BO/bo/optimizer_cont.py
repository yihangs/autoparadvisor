...
class ContOptimizer(Optimizer):
    ...
    def restart(self):
        print("Restart step...")
        #pdb.set_trace()
        from bo.localbo_utils import train_gp

        if self.guided_restart and len(self.casmopolitan._fX):
            # integer case not implemented
            # Get the best index
            ...
            fX_tr_torch = torch.tensor(best_fX_each_restart, dtype=torch.float32).view(-1)
            # Train the auxiliary
            #pdb.set_trace()
            self.auxiliary_gp = train_gp(
                ...
                cont_lb = self.casmopolitan.lb,
                cont_ub = self.casmopolitan.ub
            )
            ...
            #X_init[:, self.casmopolitan.cont_dims] = from_unit_cube(X_init[:, self.casmopolitan.cont_dims], self.lb[self.n_onehot:],self.ub[self.n_onehot:])
            ...
            #lb = np.clip(centre_cont - self.casmopolitan.length_init / 2.0, self.lb[self.n_onehot:], self.ub[self.n_onehot:])
            #ub = np.clip(centre_cont + self.casmopolitan.length_init / 2.0, self.lb[self.n_onehot:], self.ub[self.n_onehot:])
            lb = np.clip(centre_cont - self.casmopolitan.length_init / 2.0, 0.0, 1.0)
            ub = np.clip(centre_cont + self.casmopolitan.length_init / 2.0, 0.0, 1.0)
            ...
            # if default parameters are specified, then put them into initial
            if(len(self.casmopolitan.X)==0 and len(self.default_x)>0):
                self.X_init[-1] = self.default_x
    ...
    def suggest(self, n_suggestions=1):
        ...
        if(len(self.X_init)>0):
            X_next = deepcopy(self.X_init)
            self.X_init = self.X_init[len(self.X_init):,:]
            return X_next
