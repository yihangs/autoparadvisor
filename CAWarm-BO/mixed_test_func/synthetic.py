...
class Func2C_testDiscrete(TestFunction):
    """Func2C is a mixed categorical and continuous function. The first 2 dimensions are categorical,
    with possible 3 and 5 possible values respectively. The last 2 dimensions are continuous"""
    ...
    def __init__(self, lamda=1e-6, normalize=False):
        # Specifies the indices of the dimensions that are categorical and continuous, respectively
        super(Func2C_testDiscrete, self).__init__(normalize)
        self.categorical_dims = np.array([0])
        self.continuous_dims = np.array([1, 2, 3])
        self.dim = len(self.categorical_dims) + len(self.continuous_dims)
        self.n_vertices = np.array([3])
        self.config = self.n_vertices
        self.int_constrained_dims = np.array([1])
        # Specfies the range for the continuous variables
        self.lb = np.array([0,-1, -1])
        self.ub = np.array([4, 1, 1])
        self.lamda = lamda
        self.default = np.array([1,0,0.3,0.7])
    ...
    def compute(self, X, normalize=True):
        ...
        X_cat = X[:, self.categorical_dims]
        X_cont = X[:, self.continuous_dims]
        X_cont[:,0] = np.round(X_cont[:,0])
        X_cont[:,-2:] = X_cont[:,-2:] * 2

        for i, X in enumerate(X):
            if X_cat[i, 0] == 0:
                res[i] = myrosenbrock(X_cont[i, -2:])
            elif X_cat[i, 0] == 1:
                res[i] = mysixhumpcamp(X_cont[i, -2:])
            else:
                res[i] = mybeale(X_cont[i, -2:])
...
class Func3C_testDiscrete(TestFunction):
    """
    Func3C is a simlar function, but with 3 categorical variables (each of which is binary) and 2 continuous variables
    """
    problem_type = 'mixed'
    ...
    def __init__(self, lamda=1e-6, normalize=False):
        super(Func3C_testDiscrete, self).__init__(normalize)
        self.categorical_dims = np.array([0, 1])
        self.continuous_dims = np.array([2, 3, 4])
        self.dim = len(self.categorical_dims) + len(self.continuous_dims)
        self.n_vertices = np.array([2, 2])
        self.config = self.n_vertices
        self.int_constrained_dims = np.array([2])
        # Specfies the range for the continuous variables
        self.lb = np.array([0, -1, -1])
    ...
    def compute(self, X, normalize=True):
        ...
        for i, X in enumerate(X):
            ...
            if X_cont[i, 0] == 0:
                res[i] += 5 * mysixhumpcamp(X_cont[i, -2:])
            elif X_cont[i, 0] == 1:
                res[i] += 2 * myrosenbrock(X_cont[i, -2:])
            else: # should never be activated
                print("Func3C_testDiscrete Error...")
                sys.exit(1)
                #res[i] += X_cont[i, 2] * mybeale(X_cont[i, -2:])
